...
export function validateCodeForErrors(
  ...
  if (inErrsButNotFoundInTheHeader.length) {
    ...
    const filesToErrors: Record<string, Typescript.Diagnostic[]> = {}
    const noFiles: Typescript.Diagnostic[] = []
    ...
    const showDiagnostics = (title: string, diags: Typescript.Diagnostic[]) => {
      return (
        `${title}\n  ` +
        diags
          .map(e => {
...
export function parsePrimitive(value: string, type: string) {
  switch (type) {
    case "number":
      return +value
    case "string":
...
export function getIdentifierTextSpans(ts: typeof Typescript, sourceFile: Typescript.SourceFile) {
  const textSpans: { span: Typescript.TextSpan; text: string }[] = []
...
function setOption(name: string, value: string, opts: CompilerOptions, ts: typeof Typescript) {
...
export interface TwoSlashOptions {
  ...
  lzstringModule?: typeof lzstring
...
(      o: { name: string; }) => o.name === fsRoot + handbookOptions.showEmittedFile || o.name === handbookOptions.showEmittedFile
  ...
  const compressToURL = "compressToEncodedURIComponent" in lzString ? lzString.compressToEncodedURIComponent as typeof lzString.compressToURL : lzString.compressToURL;
  const zippedCode = compressToURL(originalCode)
