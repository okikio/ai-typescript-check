...
import { supportedReleases } from "./release_data.ts"
import { extractTwoSlashCompilerOptions, twoslashCompletions } from "./twoslashSupport.ts"
...
function defaultFilePath(config: SandboxConfig, compilerOptions: CompilerOptions) {
  const isJSX = compilerOptions.jsx !== Typescript.JsxEmit.None
  const ext = isJSX && config.filetype !== "d.ts" ? config.filetype + "x" : config.filetype
  return "input." + ext
}

/** Creates a monaco file reference, basically a fancy path */
function createFileUri(config: SandboxConfig, compilerOptions: CompilerOptions) {
  return new URL(defaultFilePath(config, compilerOptions), "http://localhost:3000/")
}
...
export const createTypeScriptSandbox = (
    ...
    const defaultText = config.suppressAutomaticallyGettingDefaultText
      ? config.text
      : getInitialCode(config.text, location)
    ...
    const filePath = createFileUri(config, compilerOptions)
